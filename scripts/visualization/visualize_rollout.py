...
import os
import pickle
from varyingsim.datasets.fov_dataset import StartEndDataset, SmoothFovDataset
...
def show_rollout(exp_dir, data_dir, idx, start=0, end=None):
    ...
    with open(os.path.join(exp_dir, 'params.yaml'), 'rb') as f:
        params = yaml.load(f)
    with open(os.path.join(exp_dir, 'algo_options.yaml'), 'rb') as f:
        algo_params = yaml.load(f)
    
    H = algo_params['context_size']
    dataset = StartEndDataset(data_dir, H)
    obs_skip = params.obs_skip

    smooth_dataset = SmoothFovDataset(data_dir, H, obs_skip=obs_skip)

    datum = dataset[idx]
    print('idx', idx)
    print('prefix', smooth_dataset.prefix_lens[idx])
